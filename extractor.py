...
class Extractor(object):
    def __init__(self):
        self.orb = cv2.ORB_create(edgeThreshold=15, patchSize=31,
                                  nlevels=8, fastThreshold=20,
                                  scaleFactor=1.2, WTA_K=2, 
                                  scoreType=cv2.ORB_FAST_SCORE,
                                  firstLevel=0, nfeatures=500)
        # distances are binary values 
    ...
    def extract(self, img):
        ...
        if self.last is not None:
            matches = self.bf.knnMatch(desc, self.last['desc'], k=2)
            for m,n in matches:
        ...
        if len(ret) > 0:
            ret = np.array(ret)
            models, inliers = ransac((ret[:, 0], ret[:, 1]),
